...
Nothing here is a secret. A password belonging to a machine goes to the
operating system's credential vault (`app.infrastructure.security.
secret_vault`); the one secret a build has to arrive with travels in the
provisioning bundle (`app.config.provisioning`). Neither is ever a
...
# The bundle the build was packaged with, carried inside the executable.
# Generated by `scripts/provision_build.py`, read by
# `app.config.provisioning` — which is also the only place that explains
# why one secret lives in it.
PROVISIONING_FILENAME = "provisioning.dat"


# -------------------------------------------------------- instance lock

# The lock file one copy of the application holds while it runs, so a
# second copy hands the screen back to it instead of opening beside it.
SINGLE_INSTANCE_LOCK_FILE = "ChandGraphicsERP.lock"